...
export default function DynamicPage({ initialPage, initialError, initialSettings }: DynamicPageProps) {
  ...
  const RAW_HTML_COMPONENT_TYPE = 'raw-html'
  ...
  useEffect(() => {
    if (!page) return
    const components = (page as any)?.template_data?.components
    if (!Array.isArray(components)) return
    const hasRawHtml = components.some((component: any) => component?.type === RAW_HTML_COMPONENT_TYPE)
    if (!hasRawHtml) return
    if (typeof window === 'undefined') return

    const storageKey = 'raw-html-force-reload'
